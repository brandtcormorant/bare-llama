...
test('LlamaModel loads and initializes correctly', async function (t) {
  ...
  t.alike(metadata.embedding, false, 'Should initialize as generation model by default')
...
test('LlamaModel handles text generation', async function (t) {
  ...
  const generated = await model.generate(prompt, { temperature: 0.9, maxTokens: 7 })

  // A wild guess at what the model might generate 😏
  t.ok(generated.includes('jumps over the lazy dog'), 'Should generate coherent text')
})
